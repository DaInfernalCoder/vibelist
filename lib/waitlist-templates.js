...
export function validateCustomizationData(customizationData) {
  ...
  if (
    customizationData.backgroundImageUrl &&
    typeof customizationData.backgroundImageUrl !== "string"
  ) {
    return {
      error: "Background image URL must be a string",
      status: 400,
    };
  }

  if (
    customizationData.showBackgroundImage !== undefined &&
    typeof customizationData.showBackgroundImage !== "boolean"
  ) {
    return { error: "showBackgroundImage must be a boolean", status: 400 };
  }

  if (
    customizationData.backgroundImageOpacity &&
    (isNaN(parseFloat(customizationData.backgroundImageOpacity)) ||
      parseFloat(customizationData.backgroundImageOpacity) < 0 ||
      parseFloat(customizationData.backgroundImageOpacity) > 1)
  ) {
    return {
      error: "Background image opacity must be a number between 0 and 1",
      status: 400,
    };
  }
